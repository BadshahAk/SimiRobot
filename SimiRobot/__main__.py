...
import SimiRobot.modules.sql.users_sql as sql
...
from SimiRobot.modules import ALL_MODULES
from SimiRobot.modules.helper_funcs.chat_status import is_user_admin
from SimiRobot.modules.helper_funcs.misc import paginate_modules
...
💜💜💜⇣❥deploy on VPS🔸
💛💛⇣❥love themed🔸
💚⇣❥Manage by [Andy](https://t.me/Its_King_Andy)
༺🖤༻༺🧡༻༺💛༻༺💜༻
"""
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("SimiRobot.modules." + module_name)
...
@run_async
def Simi_about_callback(update: Update, context: CallbackContext):
    query = update.callback_query
    if query.data == "Simi_":
        uptime = get_readable_time((time.time() - StartTime))
        query.message.edit_text(
...
""",
            ...
            [
                InlineKeyboardButton(
                    ...
                    [
                        InlineKeyboardButton(text="♥️ вαϲк ♥️", callback_data="Simi_back"),
                    ],
                ]
           ),
        )
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 5617001475 and DONATION_LINK:
            update.effective_message.reply_text(
                f"» ᴛʜᴇ ᴅᴇᴠᴇʟᴏᴩᴇʀ ᴏғ {dispatcher.bot.first_name} sᴏᴜʀᴄᴇ ᴄᴏᴅᴇ ɪs [ɢɪᴛʜᴜʙ](https://github.com/BadshahAk/Simi_Ro_bot)"
...
""",
    ...
    about_callback_handler = CallbackQueryHandler(
        Simi_about_callback, pattern=r"Simi_"
    )
    ...
    LOGGER.info("💖SimiRobot♥️ started with fully smoothest power💖")

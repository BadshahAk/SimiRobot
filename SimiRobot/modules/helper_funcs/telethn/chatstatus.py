...
from SimiRobot import DRAGONS
from SimiRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Simi_is_admin(chat_id: int):
    status = False
    Simi = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Simi.id == user.id:
